...
quredyi=quzhi(15,lishiqured,"max")
qublueyi=quzhi(5,lishiqublue,"max")
quredlv=quzhi(10,lishiquredlv,"min")
...
for i in xrange(len(redjiao)):
        if int(redjiao[i])<=3 or int(redjiao[i])>=30:
            if int(redjiao[i])<=3:
                if my1==0:
                    my1+=1
                else:
...
if len(isbai)==0:
    del mydb
    mydb = Source1.MyPdo('localhost','root','root','caipiao')
    sql="insert into bocai_shuangsebai VALUES (0,"+str(times)+",'"+allshujv+"')"
    if mydb.mystr(sql):
        pass
    else:
        print("error")
